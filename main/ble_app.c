...
static const esp_gatts_attr_db_t gatt_db[HRS_IDX_NB] =
{
    ...
    /* Characteristic Value */
    [IDX_CHAR_VAL_B]  =
    {{ESP_GATT_RSP_BY_APP}, {ESP_UUID_LEN_16, (uint8_t *)&CHAR_2_LONG_WR, ESP_GATT_PERM_READ | ESP_GATT_PERM_WRITE,
...
static void gap_event_handler(esp_gap_ble_cb_event_t event, esp_ble_gap_cb_param_t *param)
{
    switch (event) {
        ...
        case ESP_GAP_BLE_AUTH_CMPL_EVT: {
            ...
            memcpy(dv_addr, param->ble_security.auth_cmpl.bd_addr, sizeof(esp_bd_addr_t));
    ...
    switch (event) {
        ...
        case ESP_GATTS_READ_EVT:
            ...
            if(gatt_db_handle_table[IDX_CHAR_VAL_A] == param->read.handle) {
                ...
                ESP_LOGI(EXAMPLE_TAG,"(2) ***** Data : %s *****\n",write_data);
            }
            if(gatt_db_handle_table[IDX_CHAR_VAL_B] == param->read.handle) {
                    int data_len = sizeof(MQTT_Data);
                    esp_gatt_rsp_t rsp;
                    ...
                    {
                        rsp.attr_value.value[i] = MQTT_Data[i];
                    }
                    esp_ble_gatts_send_response(gatts_if, param->read.conn_id, param->read.trans_id,
                                    ESP_GATT_OK, &rsp);
                ESP_LOGE(EXAMPLE_TAG, "(5) ***** read char2 ***** \n");
                ESP_LOGI(EXAMPLE_TAG,"(5) ***** Data : %s *****\n",MQTT_Data);
                flag2 = true;
            }
       	    break;
        case ESP_GATTS_WRITE_EVT:
            if (!param->write.is_prep){
                ...
                if (gatt_db_handle_table[IDX_CHAR_CFG_C_2] == param->write.handle && param->write.len == 2){
                    ...
                    uint8_t notify_data[sizeof(MQTT_Data)];
                    memcpy(notify_data,MQTT_Data,sizeof(MQTT_Data));
